...
任务分析先消耗预算；完成后，系统按剩余预算冻结包含每张图片、每个 Target 和每条重试边的完整最坏调用图。
复合节点必须先原子预留视觉与首个回答容量，才允许视觉调用上网；每次视觉内部重试逐次消费独立
票据，缓存命中与未执行分支释放未用容量。每个物理调用都进入调用与成本台账，失败调用也计数，
缓存或共享命中不计调用。
...
> 回退。完整最坏调用图、复合节点原子预算预留和逐物理调用成本核算均已实现。Session 绑定已实现
> 同 Route 沿用、能力不足升级、临时故障不改绑定和自动过期。
