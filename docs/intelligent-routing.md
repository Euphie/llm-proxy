...
> 视觉计划、统一尝试预算、提交前模型切换和结构化路由轨迹；Session、策略生命周期、动态质量
> 学习和多 Target 仍是后续路线图。
...
凭据。请求缺少有效 Session ID 时，不建立跨请求绑定。

- 如果模型的上下文或能力被证明永久不足，可以把会话升级到更强模型。
...
2. **稳定运行（部分实现）**：路由轨迹与 Profile 配置热更新已上线；Session 绑定、独立策略版本、
