...
> 状态：Phase 1 已实现。当前运行时已具备 `model=auto`、本地/轻量任务判断、质量与成本选型、
> 视觉计划、统一尝试预算、提交前模型切换、Session 模型连续性和结构化路由轨迹；独立策略
> 生命周期、动态质量学习和多 Target 仍是后续路线图。
...
- 一份有效策略配置：包含任务类型到 Route 的映射、默认 Route、各 Route 的候选/质量门槛，
  以及在 Route 映射前就能创建的一份统一请求预算。Phase 1 随 Profile 原子热更新；独立发布、
  灰度和回滚生命周期属于后续阶段。
...
Phase 1 将当前 Profile 的 Upstream 与模型 ID 视为一个隐式 Target，不提供 Target 池。后续只
允许在同一 Profile、同一模型、同一供应商下增加多个 Target，用于多区域或多部署容错；仍然
不能跨供应商或跨 Profile。
...
轻量任务分析失败、超时或置信度不足时，只有当前 Profile 的强模型基线满足全部硬约束才会
回退；否则请求失败。安全限制、协议不兼容、能力不足和凭据错误属于硬失败，不能用降级绕过。
...
预算。每次视觉、回答、重试或切换也必须先占用预算。重试循环结束后不得再额外发送一次请求。
...
Phase 1 由管理员在 Route 候选中填写质量分和严重错误率，运行时把它们作为发布时已确认的门槛
事实。自动积累证据和更新估计尚未上线。

后续质量估计采用分层贝叶斯方法：小样本向整体均值收缩、旧数据按时间衰减，并使用保守置信区间；
...
1. 当前响应已经返回客户端后，任务进入有界内存队列；队列满时直接丢弃，不阻塞在线请求。
2. 系统按确定性格式检查、候选/强模型基线对照和盲化 A/B 质量评审收集证据。
3. 评测更新模型在不同 Route 上的质量与成本估计，并生成候选策略。
4. 候选策略必须由管理员检查和发布，系统不会自动替换线上策略。
...
Phase 1 已随 Profile 保存原子热加载策略配置：新请求读取新快照，已经开始的请求固定旧快照。
独立策略版本、CAS 状态切换、灰度、最近已知良好版本回滚和紧急模型停用仍未实现。目标行为是：
已发布策略不可修改，调整生成新版本；自动动作只允许回滚，启用、灰度扩大和正式发布由管理员
决定。
...
- Profile 是永久隔离边界；一个模型先对应一个 Target。
...
2. **稳定运行（部分实现）**：Session 绑定、路由轨迹与 Profile 配置热更新已上线；独立策略版本、
   CAS 发布/灰度/LKG 回滚和质量等级展示待实现。
3. **持续优化（规划）**：异步抽样评测、质量评审、候选策略，以及同 Profile 的多 Target。
