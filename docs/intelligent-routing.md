...
> 状态：Phase 1、独立策略生命周期、异步对比学习和有序多 Target 容错已实现。当前运行时已具备 `model=auto`、本地/轻量任务判断、
...
顺序配置最多 16 个备用 Target，并为每个备用项声明精确模型列表。备用端点必须属于同一供应商，
且能直接复用当前请求的凭据和协议；系统不转换凭据，也不允许跨 Profile 调度。显式模型请求仍
只使用 primary，只有 `model=auto` 会把相应模型的 primary 和备用 Target 冻结进执行计划。
...
2. **稳定运行（已实现）**：Session 绑定、路由轨迹、不可变策略版本、CAS 发布/灰度、LKG 回滚、
   原子热加载和配置质量等级已上线。
3. **持续优化（已实现）**：异步抽样评测、盲化质量评审、证据候选和同 Profile 的有序多 Target 容错。
