...
> 状态：目标设计。本文描述准备实现的智能路由，不表示当前 Go 运行时已经具备这些能力。
...
- 一个已发布策略：包含任务类型到 Route 的映射、默认 Route、各 Route 的候选/质量门槛/成本
  目标，以及在 Route 映射前就能创建的一份统一请求预算。
...
第一版每个模型只配置一个 Target。后续只允许在同一 Profile、同一模型、同一供应商下增加多个
Target，用于多区域或多部署容错；仍然不能跨供应商或跨 Profile。
...
质量估计采用分层贝叶斯方法：小样本向整体均值收缩、旧数据按时间衰减，并使用保守置信区间；
...
已发布策略不可修改；调整会生成新版本。发布使用 CAS 防止并发覆盖，运行时通过原子快照热
加载，新请求使用新策略，已经开始的请求继续使用旧快照。自动动作只允许回滚到最近一次已知
良好版本；启用、灰度扩大和正式发布仍由管理员决定。紧急禁用模型必须立即生效。
...
这些限制是第一版的明确范围，不应描述成已经完成的运行时能力。未来如果扩展到多副本，必须
先引入共享控制面、分布式预算和一致的策略发布机制，而不是共享挂载同一个 SQLite 文件。

## 12. 实施顺序

1. **可用路由**：实现 `auto`、轻量任务分析、强模型基线、视觉计划和统一尝试预算。
2. **稳定运行**：实现 Session 绑定、策略版本/热加载/灰度、路由轨迹和质量等级展示。
3. **持续优化**：实现异步抽样评测、质量评审、候选策略，以及同 Profile 的多 Target。
