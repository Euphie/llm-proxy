...
> LKG 回滚和证据生成。完整最坏调用图和跨步骤原子预算预留仍属于后续工作。
...
流式响应出现第一个完整、合法的协议事件后进入 `ClientCommit`；此后即使上游失败，也不能再
换模型或重试，以免向客户端拼接两份响应。响应后的异步评测使用独立评测预算，不占用已经
结束的在线请求预算。

当前预算会在每次实际视觉、回答、重试和切换前检查对应上限；但尚未把任务分析、各模型的每张
图片、每个 Target、主回答和所有重试边一次性展开成完整最坏调用图，也未提供覆盖整条复合边的
单次原子预留。文档中的“统一预算”不表示这项后续语义已经完成。
...
5. **完整调用图（待实现）**：预先展开最坏调用图，并为跨视觉、切换和回答的复合尝试提供原子预算
   预留与一致的计划成本证明。
