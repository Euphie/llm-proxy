...
目标从最终主请求 URL 推导：保留 Upstream 基础路径和查询参数，并把已识别的入口操作后缀规范为
...
配置的输出与单图提示预留可放入窗口。复合候选要求回答模型和视觉模型都满足所需能力。
主回答前发生连接失败、超时、临时过载或畸形视觉响应时，可在剩余预算内重试同一 Upstream 或
切换计划内模型；鉴权、请求/能力和不可重试 4xx 错误不会回退。
